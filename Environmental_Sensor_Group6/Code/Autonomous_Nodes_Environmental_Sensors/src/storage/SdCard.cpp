...
Status SdCard::begin(uint8_t csPin, uint32_t initHz, uint32_t workHz) {
  ...
  // Use default frequencies from config if not overridden
  if (initHz == 0) initHz = SD_INIT_FREQ_HZ;
  if (workHz == 0) workHz = SD_WORK_FREQ_HZ;

  // Optional: Power/Insertion stabilization delay
  #if defined(SD_POWER_DELAY_MS) && (SD_POWER_DELAY_MS > 0)
    delay(SD_POWER_DELAY_MS);
  #endif

  // If Card Detect pin exists, check with debounce
  ...
  // Speed up: Re-mount at working frequency for maximum compatibility
  if (workHz != initHz) {
    SD.end();
  ...
  // Optional: Create default log directory
  mkdirs_("/logs");
  return Status::Ok();
}
...
bool SdCard::mkdirs_(const String& fullPath) const {
  ...
  // Create directories level by level: /a /a/b /a/b/c ...
  int idx = 1;
...
Result<File> SdCard::openWrite(const char* path, bool truncate) const {
  ...
  // FILE_WRITE defaults to append; remove file first if truncation is requested
