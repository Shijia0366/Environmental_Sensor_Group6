...
// ---- Test 3: GPS (I2C, Non-blocking Poll) ----
...
static void test_tof(unsigned tries = 10) {  
  ...
  if (s_tofInitialized == 0) {
    // 1) Init
    auto st = d_tof.begin(HAL::getWire(), ToF_ADDR, TOF_XSHUT_PIN, TOF_INT_PIN);
    if (!st.ok) { Serial.println(String("  FAIL: ") + st.err); return; }

    // 2) Recommended: Explicitly configure distance mode / timing budget
    d_tof.VL53L4CX_SetDistanceMode(VL53L4CX_DISTANCEMODE_MEDIUM);
    d_tof.VL53L4CX_SetMeasurementTimingBudgetMicroSeconds(50000); // 50 ms budget

    // 3) Start & Warmup: Clear interrupt once before, then wait for one budget cycle
    if ((rc = d_tof.VL53L4CX_StartMeasurement())) { Serial.printf("  FAIL: start=%d\n", rc); return; }
    d_tof.VL53L4CX_ClearInterruptAndStartMeasurement();  
    delay(60);  // Warmup: ~timing budget (50ms) + margin
    s_tofInitialized = true;
  }

  for (unsigned i = 0; i < tries; ++i) {
    // 4) Wait for Ready (First frame is slower)
    ...
    if (rc || !NewDataReady) {
      ...
      // Soft restart measurement to prevent lockup
      d_tof.VL53L4CX_ClearInterruptAndStartMeasurement();
      d_tof.VL53L4CX_StopMeasurement();
      d_tof.VL53L4CX_StartMeasurement();
      // Give a little warmup time
      delay(60);
      continue;
    }

    // 5) Read Frame & Clear Interrupt Immediately (Critical)
    ...
    // 6) Print Results (Summarize best object)
...
// ---- Test 5: SD Card (Mount + Write + Readback) ----
static void test_sd() {
  Serial.println(F("\n[TEST] SD card (SPI)"));
...
// ---- Test 0: I2C Scan + Power Sequence Verification ----
static void test_i2c_scan() {
  Serial.println(F("\n[TEST] I2C scan + power"));
  HAL::beginI2C();  // Includes I2C power pin logic and delay
...
// ---- Public Interface: diag_setup / diag_loop (called by main) ----
void diag_setup() {
  Serial.begin(115200);
  delay(50);
  Serial.println(F("\n[diag] boot"));
...
void diag_loop() {
  ...
  if (Serial.available()) {
    ...
    switch (c) {
      ...
      case 'q': 
        Serial.println(F("Reboot to exit diag.")); 
        // Note: ToF requires XSHUT to reset, otherwise it retains state while MCU resets
        d_tof.stop(); // Attempt soft stop before reboot
        delay(200); 
        esp_restart(); 
        break; 
