...
Status OledDisplay::begin(TwoWire& w, uint8_t addr, int8_t rstPin) {
  ...
  // First try SWITCHCAPVCC; if that fails, perform a hard reset and then try EXTERNALVCC.
...
void OledDisplay::Rotate180() {
  ...
  oled_->display();          // Truly clear screen
}
...
void OledDisplay::drawOledPaged(const DataRecord& r) {
  ...
  uint32_t now = millis();
  if (now - t0 >= PAGE_MS) { t0 = now; page = (page + 1) % 5; }   //5 pages
